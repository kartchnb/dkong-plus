...
static int rc_set_defaults(struct rc_option *option)
{
   ...
   //logerror("info: rc_set_defaults start\n");
   /* set the defaults */
   ...
   //logerror("info: rc_set_defaults end\n");
...
struct rc_struct *rc_create(void)
{
   ...
   {
      logerror( "error: malloc failed for: struct rc_struct\n");
      return NULL;
   }
...
int rc_register(struct rc_struct *rc, struct rc_option *option)
{
   ...
   //logerror("info: rc_register start\n");
   ...
   if(i >= (rc->option_size-1))
   {
      ...
      if (!tmp)
      {
         logerror( "error: malloc failed in rc_register_option\n");
         return -1;
      }
      rc->option = tmp;
   ...
   int priority, int continue_on_errors)
{
   ...
   if (options.debug)
      logerror( "info: trying to parse: %s\n", name);

   if (!(f = fopen(name, "r")))
   {
      //don't print out this error, .ini files are optional
      if (options.debug)
         logerror("fopen (%s) failed\n",name);
      return 0;
   }
   ...
   int priority, int continue_on_errors)
{
   ...
   while (fgets(buf, BUF_SIZE, f)!=NULL)
   {
      ...
      {
         if (options.debug)
            logerror("info: rc_read blank line\n");
         continue;
      }
      if(name[0] == '#')
      {
         if (options.debug)
            logerror("info: rc_read comment\n");
         continue;
      }
      ...
      if (!arg)
      {
               logerror( "error: garbage \"%s\" on line %d of file: %s\n",
      ...
      if (options.debug)
         logerror("info: processing arg name (%s) arg (%s)\n",name,arg);

      if(!(option = rc_get_option2(rc->option, name)))
      {
         // Buddabing: don't print unknown option message
         if (options.debug)
            logerror("error: unknown option %s, on line %d of file: %s\n",
      ...
      {
         if (options.debug)
            logerror("   ignoring line\n");
      }
      else
         return -1;
   }
   if (options.debug)
      logerror("info: rc_read end\n");
   return 0;
}

static int real_rc_write(struct rc_option *option, FILE *f, const char *description)
{
	int i;
   ...
   int priority, int (*arg_callback)(char *arg))
{
   ...
   //logerror("info: rc_parse_commandline start, %d args\n",argc);
         ...
         if((option = rc_get_option2(rc->option, argv[i] + start)))
         {
            ...
            else
            {
               ...
               if (rc_requires_arg[option->type])
               {
                  ...
                  if (i >= argc)
                  {
                     logerror( "error: %s requires an argument\n", argv[i-1]);
                     return -1;
                  }
                  arg = argv[i];
               }
   ...
   //logerror("info: rc_parse_commandline end\n");
   return 0;
}
   ...
   int priority)
{
   ...
   if(i == (sizeof(int)*8))
   {
      logerror( "error invalid value for %s: %s\n", option->name, arg);
      return -1;
   }
...
int rc_set_option(struct rc_struct *rc, const char *name, const char *arg,
   int priority)
{
   //logerror("info: rc_set_option (%s=%s), pri %d\n",name,arg,priority);
...
{
   struct rc_option *my_option;
   //logerror("info: rc_set_option2 (%s=%s), pri %d\n",name,arg,priority);
...
int rc_set_option3(struct rc_option *option, const char *arg, int priority)
{
   ...
   //logerror("info: rc_set_option3 (arg=%s), pri %d\n",arg,priority);

   /* check priority */
   if(priority < option->priority)
   {
      //logerror("priority %d less than option priority %d\n",
      ...
      case rc_string:
         {
            ...
            {
               logerror( "error: malloc failed for %s\n", option->name);
               return -1;
            }
            strcpy(str, arg);
            if(*(char **)option->dest)
      ...
      case rc_file:
         {
            ...
            if(!f)
            {
               logerror( "error: couldn't open file: %s\n", arg);
               return -1;
            }
            if (*(FILE **)option->dest)
...
/* gimme the entire tree, I want todo all the parsing myself */
