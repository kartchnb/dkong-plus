; Adding a secret stage to Donkey Kong...
...
;------------------------------------------------------------------------------
; Program entry point
L_ORG:  xor     a                       	; Disable interrupts
...
l0010:  ld      a,(MARIO_ALIVE)
        ...
        ret     c
        pop		hl							; Return from calling function otherwise
        ret     
;------------------------------------------------------------------------------
...
l0018:  ld      hl,MINOR_TIMER				; -- MINOR_TIMER
        ...
        pop		hl							; Return from the calling function otherwise
        ret     
;------------------------------------------------------------------------------
...
l0413:  
		; If this is the secret stage, jump ahead to skip animating
		; Donkey Kong
		cp		5
		jp		z,l_animate_dk_and_pauline_1
		
		; If Donkey Kong is in an action, jump ahead to perform it
		ld      a,(TIME_FOR_DK_ACTION)
        and     a
        jr      nz,l0426
...
; and Pauline is animated as frantic
		; Prepare the location of the Help! tiles
l0486:	ld      hl,TILE_COORD(14,4)
		ld		b,$ef

l0493:  ld      a,(ANIMATION_TIMER)
        ld      c,a
...
l04a3:  ld      de,32						; Used to move tile address one column left
...
l04be:  ld      de,32						; Used to move tile address one column left
		ld      a,$10
...
l_animate_dk_and_pauline_1:
		; Increment the animation timer
		ld      hl,ANIMATION_TIMER
        inc     (hl)
		
		; Clear or display the Help! tiles next to Pauline
		ld      hl,TILE_COORD(4,28)
		ld		b,$df
		jp		l0493
;------------------------------------------------------------------------------
...
l052e:  ld      a,(de)
        ...
        djnz    l052e						; Next b
		and		a							; Clear flags
		inc		de							; Point to second byte of player's score
		ld		a,(de)
		adc		a,(hl)
		daa
		ld		(de),a
...
l0556:  ld      a,(de)
        ...
        inc     hl
        djnz    l0556						; Next b
        jr      l05da						; Display the high score
;------------------------------------------------------------------------------
...
l05d5:  cp      2
        jr      nz,L_DISPLAY_PLAYER_SCORE
l05da:  ld      de,PREV_HIGH_SCORE+2

		; Display the score
        jr      L_DISPLAY_HIGH_SCORE
...
l0600:  ld      a,(de)
        cp      $3f
        jp      z,l0026						; Exceeds relative branch
        ld      (hl),a
        pop     af
...
L_DISPLAY_CREDITS_2:  
        ...
        jp      l0583						; Exceeds relative branch
;------------------------------------------------------------------------------
...
l0691:  ld      a,(ONSCREEN_TIMER)
        ...
        and     %00001111
        add     a,10
        jp      L_AWARD_POINTS				; Exceeds relative branch
...
l06c2:  ld      (hl),$10					; Display ' '
        ...
        jr      z,l06d7						; If there are no lives left, jump ahead
...
L_IMPLEMENT_ATTRACT_MODE: 	
		...
		ld      a,(NUM_PLAYS)
        and     a
		jr		nz,L_PROMPT_FOR_START					; If coins have been entered, just show the start prompt
		ld		a,(hl)
		rst		$28
		; Jump table
...
L_PREPARE_DEMO_MODE:  
        ...
        jp      L_DISPLAY_CURRENT_STAGE_2	; Exceeds relative branch
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
; Clear the entire screen and all the sprites
...
L_DISPLAY_CURRENT_STAGE_2:  
		...
		dec		a
		jr		z,ldisplaycurrentstage1
		
		; Must be on the secret stage
		; Draw X's in elevator column
		; ld		de,13
		; ld		hl,TILE_COORD(12,13)
		; ld		c,8
; l_display_current_stage_1:
		; ld		b,19
; l_display_current_stage_2:
		; ld		(hl),$FE
		; inc		hl
		; djnz	l_display_current_stage_2

		; add		hl,de
		; dec		c
		; jr		nz,l_display_current_stage_1		
		
		; Display the secret stage
		call	L_DRAW_SECRET_CAGE
		ld		de,L_SECRET_STAGE_DATA
...
;        jp      L_WEIRD_DISPLAY_STAGE_FN	; Exceeds relative branch
   		call	L_DISPLAY_MIXER_LADDERS
		jp		L_COMPLETE_STAGE_INIT		; Exceeds relative branch
...
;------------------------------------------------------------------------------
; Draw the cage that holds Pauline on the secret stage
L_DRAW_SECRET_CAGE:  
		; Draw the 4 bars starting at (10,26)-(6,30)
		ld      hl,TILE_COORD(10,26)
		ld		c,3
		ld		de,-37
		ld		a,$7d						; Bar tile
lsecret0d4b:
...
lsecret0d4c:
		ld      b,5							; For b = 5 to 1 (each bar is 5 sprites high)

lsecret0d4e:
		ld      (hl),a
		inc     hl
		djnz    lsecret0d4e					; Next b
...
L_COMPLETE_STAGE_INIT:  
		...
		; If the current stage is the secret stage,
		; jump ahead to skip loading Donkey Kong
		ld		a,(CURRENT_STAGE)
		cp		5
		jr		z,lpaulinesecret
		...
		; If the current stage is the secret stage, jump ahead
		jr		nc,lpaulinesecret
...
lpaulinesecret:
        ld      hl,L_PAULINE_SPRITES_FACE_RIGHT
        ld      de,PAULINE_UPPER_SPRITE_X
        ld      bc,8
        ldir    
...
;		ld		de,4						; 4 bytes per sprite 
		ld		bc,TWO_BYTES(2,120)			; 2 sprites to copy, move ??? pixels
		ld		hl,PAULINE_UPPER_SPRITE_X
		call	L_MOVE_N_SPRITES
		ld		bc,TWO_BYTES(2,192)		; 2 sprites to copy, move ??? pixels
		ld		hl,PAULINE_UPPER_SPRITE_Y
		call	L_MOVE_N_SPRITES
		ret
;------------------------------------------------------------------------------
...
l0fcb:  ld      a,c
		...
		.word 	L_INIT_SECRET_STAGE_SPRITES ; 5 = setup secret stage
;------------------------------------------------------------------------------
...
l_init_barrels_stage_sprites_1:
        ...
        ld		bc,TWO_BYTES(5,28)		; 5 blocks to copy, 32 byte data structures
        call    L_LD_SPRITE_DATA_TO_STRUCT

        ; Initialize the two (invisible) collision detection sprites on either 
        ; side of Donkey Kong
        ld      hl,L_BARREL_COL_SPRITE_COORD_DATA
		; Use the rivets stage initialization code to load the 
		; collision detection sprite data
		jp		l_init_rivets_stage_sprites_1
;------------------------------------------------------------------------------
...
L_INITIAL_BARREL_DATA:  
		.byte	0						; Sprite number					; Sprite number
		.byte	0						; Palette
		.byte	2						
		.byte	2
;------------------------------------------------------------------------------
...
L_INIT_MIXER_STAGE_SPRITES:  
        ...
        ; Load the oil barrel sprite (using code from the barrels
		; stage initialization function)
        ld      hl,L_OIL_BARREL_SPRITE_DATA_2
		jp		l_init_barrels_stage_sprites_1
;------------------------------------------------------------------------------
...
l10c3:  ld      hl,L_ELEVATOR_SPRITE_DATA
        ...
        ld      (ix+14),88				; Next x coordinate
        ld      (ix+5),128				; Fireball y coordinate
        ld      (ix+15),128				; Next y coordinate
        ...
        ; Initialize the two (invisible) collision detection sprites on either 
        ; side of Donkey Kong
        ld      hl,L_ELEV_COL_SPRITE_COORD_DATA
		; Use the rivets stage initialization code to load the 
		; collision detection sprite data
		jp		l_init_rivets_stage_sprites_1
;------------------------------------------------------------------------------
...
L_INIT_RIVETS_STAGE_SPRITES:  
        ...
        ; Initialize the two (invisible) collision detection sprites on either 
        ; side of Donkey Kong
        ld      hl,L_RIVETS_COL_SPRITE_COORD_DATA
l_init_rivets_stage_sprites_1:
        ld      de,COLLISION_AREA_STRUCTS+3	; X coordinate entry
        ld      bc,TWO_BYTES(2,30)		; 2 sprites, 32 byte data structures
        call    L_COPY_COORDS_TO_STRUCTS 
        ld      hl,L_COL_SPRITE_DATA
        ld      de,COLLISION_AREA_STRUCTS+7	; Sprite number entry
        ld      bc,TWO_BYTES(2,28)		; 2 sprites, 32 byte data structures
        call    L_LD_SPRITE_DATA_TO_STRUCT
        
        ; Misc data
        ld      ix,COLLISION_AREA_STRUCTS
...
L_COL_SPRITE_DATA:	
		.byte	K_COLLISION_SPRITE_NUM
		.byte	K_COLLISION_SPRITE_PAL
		.byte	K_COLLISION_SPRITE_WIDTH
		.byte	K_COLLISION_SPRITE_HEIGHT
;------------------------------------------------------------------------------
...
L_RIVETS_COL_SPRITE_COORD_DATA:	
		.byte	K_COLLISION_1_X_RIVETS
		.byte	K_COLLISION_1_Y_RIVETS
		
		.byte	K_COLLISION_2_X_RIVETS
		.byte	K_COLLISION_2_Y_RIVETS
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
; The following coordinates define the position of the collision detection 
; sprites on either side of Donkey Kong on the barrels stage
L_BARREL_COL_SPRITE_COORD_DATA:	
		.byte	K_COLLISION_X_BARRELS
		.byte	K_COLLISION_Y_BARRELS
...
;------------------------------------------------------------------------------
; The following coordinates define the position of the collision detection 
; sprites on either side of Donkey Kong on the elevators stage
L_ELEV_COL_SPRITE_COORD_DATA:	
		.byte	K_COLLISION_X_ELEVATORS
		.byte	K_COLLISION_Y_ELEVATORS
...
;------------------------------------------------------------------------------
; Initialize the sprites for the secret stage
L_INIT_SECRET_STAGE_SPRITES: 
		; One fireball for each level number (max of 5)
		ld		hl,L_FIREBALL_SPRITE_DATA
		push	hl
		ld		a,(CP_LEVEL_NUMBER)
		cp		6
		jr		c,l_init_secret_stage_sprites_0
		; Cap the fireball number at five
		ld		a,5
				
l_init_secret_stage_sprites_0:
		ld		b,a
		ld		c,30						; 32 byte structures
		push	bc

		; Initialize the coordinates of the fireballs
		ld		hl,L_SECRET_FIREBALL_COORDS
		ld		de,FIREBALL_STRUCTS+3		; X coordinate entry
		ld		bc,TWO_BYTES(5,30)			; 5 sprites, 32 byte structures
        call    L_COPY_COORDS_TO_STRUCTS

		; Activate all the fireballs and copy the starting coordinate
		; to the next coordinate
		pop		bc
		ld		ix,FIREBALL_STRUCTS
		ld		de,32						; 32 byte data structures
l_init_secret_stage_sprites_1:
		ld		(ix+0),1
		ld		a,(ix+3)
		ld		(ix+14),a
		ld		a,(ix+5)
		ld		(ix+15),a
		add		ix,de
		djnz	l_init_secret_stage_sprites_1
		...
		ld 		ix,ELEVATOR_STRUCTS
		ld		a,1
		ld		(ix+0),a					; Elevator 1 active
		ld		(ix+16),a					; Elevator 2 active
		
        ; Initialize the coordinates of the elevator
        ld      hl,L_SECRET_ELEVATOR_COORDS
        ld      de,ELEVATOR_STRUCTS+3		; X coordinate entry
        ld      bc,TWO_BYTES(2,14)			; 2 sprites, 16 byte data structures
        call    L_COPY_COORDS_TO_STRUCTS
        
lsecret10c3:  
		ld      hl,L_ELEVATOR_SPRITE_DATA
        ld      de,ELEVATOR_STRUCTS+7		; Sprite number entry
        ld      bc,TWO_BYTES(2,12)			; 2 sprites, 16 byte data structs
        call    L_LD_SPRITE_DATA_TO_STRUCT
        ...
        ld      hl,ELEVATOR_PLATFORM_SPRITES
        ld      b,2							; 2 elevators
        ld      de,16						; 16 byte structures
        call    L_COPY_STRUCT_TO_SPRITE
        
        ; Load the prize sprites for the secret stage
        ld      hl,L_SECRET_PRIZE_SPRITE_DATA
        ld      de,PRIZE_SPRITES
        ld      c,12						; 3 sprites
        ldir    
        
		; Load the trigger sprites for the secret stage
		ld		hl,L_SECRET_TRIGGER_DATA
		ld		de,TRIGGER_SPRITES
		ld		c,16						; 4 sprites
		ldir
...
L_LOAD_BARREL_FIRE_DATA:  
        ...
        ; Load the fire's radial width
        inc     hl
        ld      a,(hl)
        ld      (ix+9),a
        
        ; Load the fire's radial height
        inc     hl
        ld      a,(hl)
        ld      (ix+10),a
        ret     
...
L_ADD_SCORE_TO_HS_TABLE:  
        ...
        ; Isolate the MS nibble of the previous byte of the score
l13da:  dec     de
        ld      a,(de)
        rrca    
        rrca    
...
l1445:  cp      (hl)
        ...
        jp      L_RESTART_GAME				; Exceeds relative branch
...
l14dc:  ld      hl,INITIALS_TIMER_DELAY
        ...
        dec     hl
        dec     (hl)
        jp      z,l15c6						; Exceeds relative branch
...
L_IMPLEMENT_STAGE_WIN_ANIM:  
		...
		; If the current stage is secret, 
		; jump ahead to animate its win animation
        ld      a,(CURRENT_STAGE)
		cp		5
		jr		z,L_IMPLEMENT_SECRET_WIN_ANIM
		
		; If the current stage is mixer or rivets, jump ahead for 
		; their win animations
        rrca    
        jr      nc,L_IMPLEMENT_MIXER_WIN_ANIM
...
L_IMPLEMENT_MIXER_WIN_ANIM:  
		...
		rrca    
        jr 		nc,L_IMPLEMENT_RIVETS_WIN_ANIM
...
;------------------------------------------------------------------------------
; Display each phase of the secret stage win animation
L_IMPLEMENT_SECRET_WIN_ANIM:
		ld		a,(WIN_ANIMATION_STATE)
		rst		$28							; Jump to local table address
		; Jump table
		.word	L_SECRET_WIN_ANIM_0
		.word	L_PAUSE_CURRENT_STATE
		.word	L_SECRET_WIN_ANIM_1
		.word	L_PAUSE_CURRENT_STATE
		.word	L_SECRET_WIN_ANIM_2
		.word	L_PAUSE_CURRENT_STATE
		.word	L_SECRET_WIN_ANIM_3
		.word	L_ADVANCE_TO_NEXT_STAGE
;------------------------------------------------------------------------------
...
L_WIN_ANIM_DISPLAY_PAULINE:  
		...
		; If not advancing to the secret stage, then jump ahead
		; to skip displaying the question mark next to Pauline
		ld		a,(ACTIVATE_SECRET_STAGE)
		and		a
		jr		z,l_win_anim_display_pauline_1
		
		; Display question mark next to Pauline
		; (This is for advancement to the secret stage)
		ld		hl,TILE_COORD(16,4)
		ld		(hl),$FB					; Question mark
		
		; Jump ahead to skip displaying the heart sprite
		jr		l_win_anim_display_pauline_2
		
l_win_anim_display_pauline_1:		
...
L_WIN_STAGE_ANIM_3:  
		...
		; If not advancing to the secret stage, then jump ahead
		; to skip displaying the exclamation marks
		ld		a,(ACTIVATE_SECRET_STAGE)
		and		a
		jr		z,l_win_stage_anim_3_1
		
		; Display excalamation marks where the question mark was
		; (This is for advancement to the secret stage)
		ld		hl,TILE_COORD(16,4)
		ld		(hl),$37					; Question mark

		ret
		
l_win_stage_anim_3_1:
...
l1785:  ld      a,(hl)
        and     a
        jp      nz,l0026					; Exceeds relative branch
...
L_ADVANCE_TO_NEXT_STAGE:  
		...
		; If the secret stage does not need to be activated,
		; jump ahead
		ld		a,(ACTIVATE_SECRET_STAGE)
		and		a
		jr		z,l_advance_to_next_stage_1
		
		; Activate the secret stage next
		ld		a,5
		
		; Force the player's height to advance by forcing a
		; mismatch between CP_STAGE_ORDER_POINTER_LAG and
		; CP_STAGE_ORDER_POINTER
		ld		hl,CP_STAGE_ORDER_POINTER_LAG
		dec		(hl)
		
		jr		l17a0

l_advance_to_next_stage_1:		
		; Advance to the next stage
        ld      hl,(CP_STAGE_ORDER_POINTER)
        inc     hl
        
        ; If the end of the stage order data has not been reached, 
		; jump ahead
        ld      a,(hl)
        cp      $7f
        jr      nz,l_advance_to_next_stage_2
        
        ; Repeat level 5's stage order indefinately
        ld      hl,L_STAGE_ORDER_TABLE_LAST
        ld      a,(hl)
	
l_advance_to_next_stage_2:	
		; If not advancing from the secret stage,
		; jump ahead
		ld		a,(CURRENT_STAGE)
		cp		5
		ld		a,(hl)
		...
		; Prevent the player's height from advancing by forcing
		; CP_STAGE_ORDER_POINTER_LAG and CP_STAGE_ORDER_POINTER
		; to match
		ld		(CP_STAGE_ORDER_POINTER_LAG),hl		
        
        ; Update the current stage
l179d:  ld      (CP_STAGE_ORDER_POINTER),hl
l17a0:  ld      (CURRENT_STAGE),a
...
L_SECRET_WIN_ANIM_0:	
		...
		; Move the heart sprite into the correct position
		ld		hl,HEART_SPRITE_X
		ld		(hl),208
		inc		hl
		inc		hl
		...
		; Prepare to have Donkey Kong's arm move to grab Pauline
		ld		a,248
		ld		(DK_REACHING_COUNTER),a
...
L_SECRET_WIN_ANIM_1:	
		...
		; Display Donkey Kong's arm grabbing for Pauline
		ld		hl,DK_REACHING_COUNTER
		dec		(hl)
		ld		a,(hl)
		cp		231
		...
		; Return if the arm hasn't reached Pauline yet
		ret		nz
...
;------------------------------------------------------------------------------
L_SECRET_WIN_ANIM_2:	
		; Change the heart to a broken heart
		ld		hl,HEART_SPRITE_NUM
		ld		(hl),$77

		; Display Donkey Kong's arm grabbing Pauline
		ld		hl,DK_SPRITE_1_NUM
		ld		(hl),$B1

		; Display Pauline in Donkey Kong's arm
		ld		hl,DK_SPRITE_2_X
		ld		(hl),234
		inc		hl
		ld		(hl),$14
		...
		; Remove the old Pauline sprites
		xor     a
		ld		(PAULINE_UPPER_SPRITE_X),a
		ld		(PAULINE_LOWER_SPRITE_X),a
...
;------------------------------------------------------------------------------
L_SECRET_WIN_ANIM_3:	
		; Display Donkey Kong pulling Pauline off-screen
		ld		hl,DK_REACHING_COUNTER
		inc		(hl)
		ld		a,(hl)
		cp		248
		...
		; Hide Donkey Kong and Pauline
		ld		b,4
		ld		hl,PAULINE_UPPER_SPRITE_X
		ld		de,4
l_secret_win_anim_3_1:		
		ld		(hl),0
		add		hl,de
		djnz	l_secret_win_anim_3_1
...
L_RUN_GAME:  
		...
		call	L_IMPLEMENT_SECRET_ELEVATORS
        call    L_IMPLEMENT_CONVEYERS
        call    L_CHECK_IF_PRIZE_REACHED
		call	L_CHECK_IF_TRIGGER_REACHED
...
;
; return:	a - 1 if a prize was reached, 0 otherwise
L_CHECK_IF_PRIZE_REACHED:  
		...
		; jump ahead
        ld      hl,PRIZE_SPRITES
        ld      b,4							; For b = 3 to 1 (3 prize sprites)
...
;------------------------------------------------------------------------------
; Check if a cage trigger has been reached on the secret stage
L_CHECK_IF_TRIGGER_REACHED:
		; Return if this is not the secret stage
		ld		a,%00010000
		rst		$30
		
		; Use the prize collision detection routine to detect
		; collision with the trigger sprites
		ld		b,4							; 4 trigger sprites
		ld		hl,TRIGGER_SPRITES
		call	lcheckifreached
		
		; If a trigger was not reached, return
		dec		a
		ret		nz
		
		; Reverse the direction of the elevators
		inc		a
		ld		(SECRET_ELEVATOR_REVERSE),a
		
		; Determine the starting coordinate of the right most cage
		; bar 
		ld		hl,CAGE_BARS_REMAINING
		ld		b,(hl)
		dec		(hl)
		ld		hl,TILE_COORD(11,26)
		ld		de,-32

l_check_if_trigger_reached_1:
		add		hl,de
		djnz	l_check_if_trigger_reached_1
		
		; Remove the right most cage bar
		ld		a,$10						; Blank tile
		jp		lsecret0d4c					
;------------------------------------------------------------------------------
...
l1ac3:  
		...
		ld      a,(MARIO_IS_JUMPING)
        dec     a
        jp      z,l1bb2						; Exceeds relative branch
        ...
        ld      a,(PLAYER_INPUT)
        rla     
        jp      c,l1b6e						; Exceeds relative branch
...
l1ae6:  call    L_IMPLEMENT_BARRIERS
        ...
        bit     0,a							; right input
        jp      nz,l1c8f					; Exceeds relative branch
...
l1af5:  dec     d
        ...
        bit     1,a
        jp      nz,l1cab					; Exceeds relative branch
...
l1b38:  ld      a,(PLAYER_INPUT)
        bit     3,a
        jp      nz,l1cf2					; Exceeds relative branch
...
l1b45:  ld      a,(PLAYER_INPUT)
        bit     2,a
        jp      nz,l1d03					; Exceeds relative branch
        ret     
...
l1b55:  ld      hl,MARIO_HAMMER_CYCLE_DELAY
        ...
        ; and return
        jp      l1da6						; Exceeds relative branch
...
l1b6e:  ld      a,1
		...
		; Make sure the Mario on elevator flag is cleared
		xor		a
		ld		(MARIO_ON_ELEVATOR),a ; SECRET STAGE
        ...
        ld      bc,$ff80
        rra     
        jp      c,l1b8a						; Exceeds relative branch
...
l1c05:  call    l2b1c
		...
		; jump ahead to update Mario's coordinate and sprite data
        ; and return
        jp      z,l1da6						; Exceeds relative branch
...
l1c76:  ld      a,(MARIO_Y)
        ...
        ; Jump ahead to update Mario's coordinate and sprite data
        ; and return
        jp      c,l1da6						; Exceeds relative branch
...
l1cf2:  ld      a,($620f)
        and     a
        jp      nz,l1d8a					; Exceeds relative branch
...
L_GIVE_POINT_AWARD:  
        ...
        rra    
        jp      c,DETERMINE_POINT_AWARD_AMT	; Exceeds relative branch
...
L_CHECK_FOR_WIN_CONDITIONS:
		; Jump ahead if this is the rivets stage or secret stage
		ld      a,(CURRENT_STAGE)
        bit     2,a
        jr      nz,l1e80		
		
		; Jump ahead if this is the barrels stage or elevators stage
        rra     
        ld      a,(MARIO_Y)
        jr      c,l1e7a						
...
l1e7a:  ld		a,(MARIO_X)
		...
		cp		49
		ret		nc
		jr		l1e6d						; Face Mario left and begin win animation

		; Return if Mario is to the right of Pauline's platform
l1e7b:	ret		nc
		
		; Check if Mario has reached the top of the escape ladders
		ld		a,(MARIO_Y)
		cp		25
		ret		nc
		
		; Activate the secret stage
		ld		a,1
		ld		(ACTIVATE_SECRET_STAGE),a
		jr		l1e85						; Trigger stage win
		
l1e80:  
		; If this is the secret stage,
		; jump ahead
		bit		0,a
		jr		nz,l1e86
...
l1e86:	
		; If Mario has passed the cage bars, then win
		ld		a,(MARIO_Y)
		cp		216
		ret		c
		ld		a,(MARIO_X)
...
l1f93:  ld      a,(ix+1)
        dec     a
        jp      z,l20ec						; Exceeds relative branch
        ...
        ; Jump ahead
        jp      l2053						; Exceeds relative branch
...
l1fac:  exx     
        ...
        xor     %00000111
        ld      (ix+2),a
        jp      L_LD_SPRITE_DATA_TO_STRUCT_2	; Exceeds relative branch
...
l1ff6:  ld      h,(ix+3)					; X coordinate entry
        ...
        and     %00000111
        cp      3
        jp      z,l215f						; Exceeds relative branch
        ...
        jr      c,l202f
        cp      $e4
        jp      c,L_LD_SPRITE_DATA_TO_STRUCT_2	; Exceeds relative branch
        xor     a
        ld      (ix+16),a
        ld      (ix+17),$60
        jr      l2038
...
l2104:  ld      a,(ix+3)
        add     a,8
        cp      16
        jp      nc,l1fce					; Exceeds relative branch
...
l2153:  ld      (ix+20),a
        ld      (ix+4),a
        ld      (ix+6),a
l215c:  jr      L_LD_SPRITE_DATA_TO_STRUCT_2
l215f:  ld      a,l
l2160:  add     a,5
        ld      d,a
        ld      a,h
        call    l216d
        jr      L_LD_SPRITE_DATA_TO_STRUCT_2
...
L_LD_SPRITE_DATA_TO_STRUCT_2:  
        ...
        jp      l1f8d						; Exceeds relative branch
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
; Implement the logic for the retracting ladders on the mixer stage
L_IMPLEMENT_RET_LADDERS:  
...
L_CHECK_IF_ON_LADDER:
		; Load bc with the number of normal ladders + 1
		ld		bc,21
...
L_IMPLEMENT_BARRIERS:  
		...
		; If this is the secret stage,
		; jump ahead to implement its barriers
        ld      a,(CURRENT_STAGE)
		cp		5
        ret     nz
		
		; Implement the cage bar barrier on the secret stage
		; Mario can't move past the bars unless they are all down
lcagebarrier:
		ld		a,(MARIO_X)
		cp		170
		ret		c
...
L_PARSE_STAGE_LADDER_DATA:  
		...
		; If the current stage is the rivets,
		; point to the rivets stage data
		dec		a
		ld		hl,L_RIVETS_STAGE_DATA
		jr		z,l2471
        
        ; Point to the secret stage data
        ld      hl,L_SECRET_STAGE_DATA
...
l24d0:  ld      (ix+0),a					; Active
        ...
        ld      a,(BARREL_FIRE_STATUS)
        and     a
        jp      nz,L_LD_SPRITE_DATA_TO_STRUCT_2	; Exceeds relative branch
...
l259a:  bit     0,(ix+0)
        ...
        ; If the pie is off the left edge of the screen, 
		; jump ahead
        ld      a,(ix+3)					; x coordinate
        ld      c,a
        add     a,7
        ...
        ; The pie is on the bottom conveyer
        ; Move the pie along the bottom conveyer
        ld      a,(BOT_CONVEYER_DIR)
        add     a,c
        ld      (ix+3),a					; x coordinate
        
        ; Move on to the next pie and sprite
l25bb:  add     ix,de
		inc		l
		inc		l
		inc		l
...
; These two conveyer belts always move inwards.  
;
; It also calls the animate motor function to animate the top two conveyer belt
; motor sprite to simulate them rotating.
...
;------------------------------------------------------------------------------
; The directions to move the elevators in each quadrant
L_QUADRANT_DIRECTIONS:
		.byte	 1,  1	; Quadrant 0
		.byte	-1,  1	; Quadrant 1
		.byte	-1, -1	; Quadrant 2
		.byte	 1, -1	; Quadrant 3
...
;------------------------------------------------------------------------------
; Handle the elevators on the secret stage
; Move the elevators
; Detect when Mario is on an elevator
; Move Mario with the elevator
; Kill Mario if he reaches the bottom of the screen or is crushed at the top 
;	of the elevator
; Make Mario fall if he steps off the elevator
;
; NOTE: Look at streamlining this function
L_IMPLEMENT_SECRET_ELEVATORS:  
		; Return unless this is the secret stage
		ld      a,%00010000
        rst     $30	
        ...
        ; If Mario has reached the bottom of the screen,
        ; jump back up to code that will mark him as dead
        ld      a,(MARIO_Y)
        cp      248
        jp      nc,l277f					; Exceeds relative branch

		; Only move the elevators every other counter cycle
        ld      a,(COUNTER_2)
		rrca	
		ret		nc
		
		; Only move the elevators when they are not paused
		ld		a,(SECRET_ELEVATOR_PAUSE_COUNT)
		and		a
		jr		z,l_implement_secret_elevators_1
		
		; Decrement the pause counter and return
		ld		hl,SECRET_ELEVATOR_PAUSE_COUNT
		dec		(hl)
		ret

l_implement_secret_elevators_1:		
		; Get the vectors for the current quadrant
		ld		hl,SECRET_ELEVATOR_QUAD
		ld		a,(hl)
		ld		hl,L_QUADRANT_DIRECTIONS
		add		a,a
		add		a,l
		ld		l,a
		ld		b,(hl)
		...
		ld		hl,SECRET_ELEVATOR_MOVE_COUNT
		inc		(hl)
		ld		a,(hl)
		
		; If the elevator has not reached the end of the quadrant,
		; jump ahead to finish
		cp		24
		jr		c,l_implement_secret_elevators_3
		
		; If the elevators don't need to reverse direction
		ld		hl,SECRET_ELEVATOR_REVERSE
		ld		a,(hl)
		and		a
		ld		(hl),0

		ld		a,(SECRET_ELEVATOR_DIR)
		jp		z,l_implement_secret_elevators_2

		; Reverse the elevator direction
		neg		

		; Change to the opposite quadrant
		push	af
		ld		hl,SECRET_ELEVATOR_QUAD
		add		a,(hl)
		ld		(hl),a
...
l_implement_secret_elevators_2:
		; Advance the quadrant in the correct direction
		ld		(SECRET_ELEVATOR_DIR),a
		ld		hl,SECRET_ELEVATOR_QUAD
		add		a,(hl)
		and		%00000011					; Force quadrant to 0-3
		ld		(hl),a						; Elevator quadrant

		; Pause for a few cycles at the end of the quadrant
		ld		a,16
		ld		(SECRET_ELEVATOR_PAUSE_COUNT),a
		
		; Reset the move counter
		xor		a
		
l_implement_secret_elevators_3:
		ld		(SECRET_ELEVATOR_MOVE_COUNT),a	; Move counter
		
		; Move the elevators
		ld		ix,ELEVATOR_STRUCT_1
		ld		de,ELEVATOR_PLATFORM_SPRITE_1
		call	l_implement_secret_elevators_5
		ld		ix,ELEVATOR_STRUCT_2
		ld		de,ELEVATOR_PLATFORM_SPRITE_2
		
l_implement_secret_elevators_5:		
		; If Mario is jumping,
		; skip the code that moves him with the elevator
		ld		a,(MARIO_IS_JUMPING)
		or		a
		jr		nz,lelevatormove
		
		; If Mario is riding an elevator
		ld		a,(MARIO_ON_ELEVATOR)
		or		a
		jr		z,lelevatormove
		
		; If Mario is on this elevator
		ld		a,(MARIO_Y)
		add		a,12
		sub		(ix+5)
		jr		nz,lelevatormove
		
		; If Mario has walked off the left edge of the elevator,
		; jump to code that will mark him as falling
		ld		a,(MARIO_X)
		sub		(ix+3)
		cp		-12
		jp		m,l2766
	
		; If Mario has walked off the right edge of the elevator,
		; jump to code that will mark him as falling
		cp		12
		jp		p,l2766		
...
lelevatormove:
		; Move the elevator vertically
		ld		a,(ix+5)
		add		a,c
		ld		(ix+5),a
		
		; Move the elevator horizontally
		ld		a,(ix+3)
		add		a,b
		ld		(ix+3),a
        
lsecret2734:  
		; Update the elevator sprites
		ld      a,(ix+3)					; X coordinate
        ld      (de),a						; Sprite x coordinate
        inc     e
        inc     e
        inc     e							; Sprite y coordinate
        ld      a,(ix+5)					; Y coordinate
        ld      (de),a
        ret     
;------------------------------------------------------------------------------
...
L_IMPL_ENEMY_COLL_FOR_STAGE:  
		...
		.word	L_RIVETS_ENEMY_COLLISION	; 5 = secret stage (same as rivets)
;------------------------------------------------------------------------------
...
L_BARREL_ENEMY_COLLISION:  
		...
		; Jump to the rivets collision detection code to check for
		; fireballs and collision areas
		jr		l_rivets_enemy_collision_1
;------------------------------------------------------------------------------
...
L_RIVETS_ENEMY_COLLISION:  
		...
		; Return if there has been a collision with a fire ball or
		; the collision areas around Donkey Kong
l_rivets_enemy_collision_1:
        ld      b,7							; 5 fire balls + 2 collision areas
        ld      a,b
        ld      (NUM_ENEMIES_OF_CURRENT_CLASS),a
        ld      de,32						; 32 byte data size
        ld      ix,FIREBALL_STRUCTS
...
L_INTERACT_WITH_ELEVATORS:  
		; Return unless this is the elevators or secret stage
		ld      a,%00010100
        rst     $30		
...
l2d2f:  ld      a,c
		...
		; Move the collision area to match Donkey Kong's position
		pop	de
		ld	a,e
		cp	0
		ld	a,K_COLLISION_X_BARRELS+16
		jr	z,l2d2f_1
		ld	a,K_COLLISION_X_BARRELS

l2d2f_1:
		ld	(COLLISION_AREA_STRUCT_1+3),a
		ld	(COLLISION_AREA_SPRITE_1),a
        ld      hl,$638f
        dec     (hl)
        jr      nz,l2d51
        ld      a,1
...
l2da5:  ld      (ix+0),$01
		...
		; Move the collision area to match Donkey Kong's new
		; position
		ld		a,K_COLLISION_X_BARRELS
		ld		(COLLISION_AREA_STRUCT_1+3),a
		ld		(COLLISION_AREA_SPRITE_1),a
...
l2e29:  ld      a,(ix+13)					; Spring state
        cp      4
        jr      z,l2e84						; If the spring is falling, skip bounce processing
        ...
        cp      $7f
        jr      z,l2e9c						; If the end of the bounce sequence has been reached, restart the sequence
...
l2e4b:  ld      (ix+14),l
        ...
        ld      a,(ix+3)					; X coordinate
        cp      183
        jr      c,l2e6c						; If the spring has not reached the gap, simply update the sprite position
...
l2e9c:  ld      hl,L_SPRING_VECTOR_DATA	
        ...
        jr      l2e4b						; Continue the bounce sequence processing
...
l2eed:  ld      (ix+14),0					; X offset
        ...
        ld      a,(HAMMER_CYCLE_ACTIVE)
        rrca    
        jp      nc,l2f97					; Exceeds relative branch
...
;------------------------------------------------------------------------------
l2fcb:  
		; Return unless this is the mixer stage, elevators stage, rivets stage,
		; or secret stage
		ld      a,%00011110
        rst     $30			
...
l316a:  add     ix,de
		...
		; is not being used
l317c:  ; If this is not the mixer stage, jump ahead
        ld      a,(CURRENT_STAGE)
        cp      2
        jr      nz,l3195
...
L_POINT_AWARD_TABLE:  
		...
		.byte	$00	; Entry 0 (000000)
		
        .byte	$01	; Entry 1 (000100)
		
		.byte	$02	; Entry 2 (000200)
		
		.byte	$03	; Entry 3 (000300)
		
		.byte	$04	; Entry 4 (000400)
		
		.byte	$05	; Entry 5 (000500)
		
		.byte	$06	; Entry 6 (000600)
		
		.byte	$07	; Entry 7 (000700)
		
		.byte	$08	; Entry 8 (000800)
		
		.byte	$09	; Entry 9 (000900)
		
		.byte	$00	; Entry 10 (001000)
		
		.byte	$10	; Entry 11 (001100)
		
		.byte	$20	; Entry 12 (001200)
		
		.byte	$30	; Entry 13 (001300)
		
		.byte	$40	; Entry 14 (001400)
		
		.byte	$50	; Entry 15 (001500)
		
		.byte	$60	; Entry 16 (001500)
		
		.byte	$70	; Entry 17 (001700)
		
		.byte	$80	; Entry 18 (001800)
		
		.byte	$90	; Entry 19 (001900)
;------------------------------------------------------------------------------
...
L_STRING_TABLE:	
		...
		.word	L_PLUS_STRING_DATA	; 29 = "PLUS!"
...
;------------------------------------------------------------------------------
; Stage order table defining the stages and the order they will appear in each 
; level
; Level 1
L_STAGE_ORDER_TABLE: 	
...
;------------------------------------------------------------------------------
; Stage data for the secret stage
L_SECRET_STAGE_DATA:
		; Ladders
		NRM_LAD_STAGE_DATA(3,25,3,31)	; Bottom left
		BRK_LAD_STAGE_DATA(28,25,28,31)	; Bottom right
		BRK_LAD_STAGE_DATA(9,19,9,25)	; Lower mid left
		NRM_LAD_STAGE_DATA(21,19,21,25)	; Lower mid right
		BRK_LAD_STAGE_DATA(3,13,3,19)	; Upper mid left
		NRM_LAD_STAGE_DATA(28,13,28,19)	; Upper mid right
		NRM_LAD_STAGE_DATA(9,7,9,13)	; Top left
		BRK_LAD_STAGE_DATA(21,7,21,13)	; Top right

		; Top platform
		STAGE_DATA(SLP_GIRDER_TILE,20,7,10,7)
		
		; Upper middle platforms
		STAGE_DATA(SLP_GIRDER_TILE,11,13,2,13)
		STAGE_DATA(SLP_GIRDER_TILE,30,13,20,13)

		; Mid middle platforms
		STAGE_DATA(SLP_GIRDER_TILE,11,19,2,19)
		STAGE_DATA(SLP_GIRDER_TILE,30,19,20,19)

		; Lower middle platforms
		STAGE_DATA(SLP_GIRDER_TILE,11,25,2,25)
		STAGE_DATA(SLP_GIRDER_TILE,28,25,20,25)

		; Bottom platforms
		STAGE_DATA(SLP_GIRDER_TILE,11,31,2,31)
		STAGE_DATA(SLP_GIRDER_TILE,30,31,20,31)
...
L_LARGE_DK_LETTER_DATA:	
		; 'D'
		.byte	5
		.word	TILE_COORD(28,7)
		.byte	1
		.word	TILE_COORD(27,7)
		.byte	1
		.word	TILE_COORD(27,11)
		.byte	3
		.word	TILE_COORD(26,8)
		
		; 'O'
		.byte	5 
		.word	TILE_COORD(24,7) 
		.byte	1 
		.word	TILE_COORD(23,7) 
		.byte	1 
		.word	TILE_COORD(23,11) 
		.byte	5 
		.word	TILE_COORD(22,7) 

		; 'N'
		.byte	5 
		.word	TILE_COORD(20,7) 
		.byte	2 
		.word	TILE_COORD(19,8) 
		.byte	2 
		.word	TILE_COORD(18,9) 
		.byte	5 
		.word	TILE_COORD(17,7) 

		; 'K'
		.byte	5 
		.word	TILE_COORD(15,7) 
		.byte	1 
		.word	TILE_COORD(14,9) 
		.byte	3 
		.word	TILE_COORD(13,9) 
		.byte	1 
		.word	TILE_COORD(12,7) 
		.byte	1 
		.word	TILE_COORD(12,11) 

		; 'E'
		.byte	5 
		.word	TILE_COORD(10,7) 
		.byte	1 
		.word	TILE_COORD(9,7)
		.byte	1 
		.word	TILE_COORD(9,9)
		.byte	1 
		.word	TILE_COORD(9,11)
		.byte	1 
		.word	TILE_COORD(8,7)
		.byte	1 
		.word	TILE_COORD(8,9)
		.byte	1 
		.word	TILE_COORD(8,11)
		...
		; 'K'
		.byte	5 
		.word	TILE_COORD(25,14)
		.byte	5 
		.word	TILE_COORD(24,14)
		.byte	2 
		.word	TILE_COORD(23,15)
		.byte	2 
		.word	TILE_COORD(22,14)
		.byte	2 
		.word	TILE_COORD(22,17)

		; 'O'
		.byte	5 
		.word	TILE_COORD(20,14)
		.byte	5 
		.word	TILE_COORD(19,14)
		.byte	1 
		.word	TILE_COORD(18,14)
		.byte	1 
		.word	TILE_COORD(18,18)
		.byte	5 
		.word	TILE_COORD(17,14)

		; 'N'  
		.byte	5 
		.word	TILE_COORD(15,14)
		.byte	2 
		.word	TILE_COORD(14,15)
		.byte	2 
		.word	TILE_COORD(13,16)
		.byte	5 
		.word	TILE_COORD(12,14)

		; 'G'
		.byte	3 
		.word	TILE_COORD(10,15)
		.byte	5 
		.word	TILE_COORD(9,14)
		.byte	1 
		.word	TILE_COORD(8,14)
		.byte	1 
		.word	TILE_COORD(8,18)
		.byte	1 
		.word	TILE_COORD(7,14)
		.byte	1 
		.word	TILE_COORD(7,16)
		.byte	1 
		.word	TILE_COORD(7,18)
		.byte	2 
		.word	TILE_COORD(6,16)
...
L_INITIAL_STAGE_DATA_TABLE:	
		...
		.byte	1	; LEFT_MASTER_CONVEYER_DIR, SECRET_ELEVATOR_DIR
		.byte	255	; BOT_MASTER_REVERSE_TIMER
		.byte	-1	; BOT_MASTER_CONVEYER_DIR (moving left)
		.byte	52	; ELEVATOR_SPAWN_TIMER
		.word	l39c3	; $62A8
		.word	BARREL_STRUCTS	; $62AA
		.word	SPRING_SPRITES	; $62AC
		.byte	26	; $62AE
		...
		.byte	0	; $62B4
		.byte	0	; $62B5, 
		.byte	0	; $62B6, SECRET_ELEVATOR_MOVE_COUNT
		.byte	0 	; $62B7, SECRET_ELEVATOR_QUAD
		.byte	4 	; BARREL_FIRE_FREEZE, CAGE_BARS_REMAINING
		.byte	0 	; BARREL_FIRE_STATE, SECRET_ELEVATOR_PAUSE_COUNT
		.byte	16 	; BARREL_FIRE_FLARE_UP_TIME
		.byte	0 	; $62BB, SECRET_ELEVATOR_REVERSE
		.byte	0 	; $62BC
		.byte	0 	; $62BD
		.byte	0 	; $62BE
		.byte	0 	; $62BF
...
;------------------------------------------------------------------------------
; Initial coordinates for the fireballs on the secret stage
L_SECRET_FIREBALL_COORDS:
		.byte	216	; X coordinate
		.byte	144	; Y coordinate

		.byte	48	; X coordinate
		.byte	192	; Y coordinate
		
		.byte	150	; X coordinate
		.byte	48	; Y coordinate
		
		.byte	80	; X coordinate
		.byte	144	; Y coordinate
		
		.byte	88	; X coordinate
		.byte	48	; Y coordinate
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
; Sprite data for the cage triggers on the secret stage
L_SECRET_TRIGGER_DATA:
		.byte	212		; x coord
		.byte	$62		; sprite number ("Button")
		.byte	$0F		; palette
		.byte	192		; y coord
...
L_ELEVATOR_COORDS:	
		...
		.byte	119	; X coordinate
		.byte	216	; Y coordinate
;------------------------------------------------------------------------------



;------------------------------------------------------------------------------
; The initial coordinates for the elevators on the secret stage
L_SECRET_ELEVATOR_COORDS:
		.byte	127	; X coordinate
		.byte	204	; Y coordinate

		.byte	127	; X coordinate
		.byte	108	; Y coordinate
;------------------------------------------------------------------------------
...
DETERMINE_POINT_AWARD_AMT: 	
		...
		rra
		jp		nc,l1e28					; Exceeds relative branch
...
;		ld		b,$7f						; 800 point award sprite
		ld		b,$7e
		jp		l1e28						; Exceeds relative branch
;------------------------------------------------------------------------------



;------------------------------------------------------------------------------
; Detect collisions with enemies on the current stage
l3e88: 	ld		a,(CURRENT_STAGE)
		dec		a
		push	hl
		rst		$28
		; Jump table
